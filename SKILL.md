...
3. Вывести ответ по формату ниже
4. После 5 ответов → итог: `X/5 (XX%)`

**НИКОГДА не генерировать вопросы.** Только обратная связь после ответа.

**Формат ответов на callback:**

✅ Правильный ответ:
```
✅ Правильно! [Краткий вывод — что закрепил]

Вопрос #[ID]: [текст вопроса]

Правильный ответ: [буква]) [текст]
Твой ответ: [буква] ✅
...
[explanation из JSON — полное объяснение базы]

───

💡 [Своя краткая мнемоника / визуализация / почему другие варианты неправильны]

───

Прогресс Quiz #[N]: [M]/5 ответов. Ещё [5-M]! 🎯
```

❌ Ошибка:
```
❌ Ошибка! [Краткий вывод — что нужно запомнить]
...
[Мнемоника / аналогия / визуализация — чтобы не ошибиться в следующий раз]
...
- Краткий вывод в первой строке (что закрепил / запомни)
- Полное `explanation` из JSON (не сокращать!)
- Своя мнемоника / визуализация (живая, не шаблонная)
- Прогресс квиза в конце (M/5)
